...
void updateWorld()
{
	...
	for (i = 0; i < kNumBalls; i++)
        ...
        {
            // YOUR CODE HERE
            n = VectorSub(ball[i].X, ball[j].X); // center to center
            float distance = Norm(n);
//            float distanceBetween = distance - (2*kBallSize); // if size is radius

            if (distance <= 2 * kBallSize && (DotProduct(ball[i].v, n) < DotProduct(ball[j].v, n)))
            {
...
void renderBall(int ballNr)
{
    ...
    tmpMatrix = Mult(modelToWorldMatrix, tmpMatrix);
