...
void DeformCylinder()
{
  ...
  int row, corner;
    ...
    mat4 T_rest[kMaxBones];
    mat4 inverse[kMaxBones];
    mat4 M[kMaxBones];
    
    //Modell till benkoordinater
    mat4 Mmb[kMaxBones];
    //Ben till modellkoordinater
    mat4 Mbm[kMaxBones];
    
    T_rest[0] = T(g_bones[0].pos.x, g_bones[0].pos.y, g_bones[0].pos.z);
    
    // skillnader i positionerna fšr translationer
    for (int i = 1; i < kMaxBones; i++) {
        T_rest[i] = T(g_bones[i].pos.x - g_bones[i-1].pos.x, g_bones[i].pos.y - g_bones[i-1].pos.y, g_bones[i].pos.z - g_bones[i-1].pos.z);
    }
    
    // Modellkoord
    for (int i = 0; i < kMaxBones; i++) {
        // g_boneRes[i].rot = r_anim
        // g_bones[i].rot = r_rest
        primes[i] = Mult(T_rest[i], g_bonesRes[i].rot); // M_bone
        inverse[i] = InvertMat4(T_rest[i]);
    }
    
    // Animated bones
    M[0] = Mult(primes[0], inverse[0]);
    for (int i = 1; i < kMaxBones; i++) {
        M[i] = Mult(Mult(primes[i], M[i-1]), inverse[i]);
    }
    vec3 b[kMaxBones];
    vec3 b_w[kMaxBones];
    vec3 result;
